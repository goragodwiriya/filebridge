...
return [
    ...
    // Files copied in parallel per job (1 = one at a time). One is the safe
    // default because a server that drops the third session is more common than
    // one that rewards parallelism, and this value applies to every connection
    // that has not set its own. Raise it per connection in the edit dialog for
    // the servers that can take it. The slower side of a transfer wins - see
    // TransferManager::workerCount().
    'transfer_workers' => 1,
